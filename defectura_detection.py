...
Дефектура = остаток ГК < DEFECT_EO_PCT * ЭО (по умолчанию 10%)
Если для КАГ нет ЭО — используется старая логика (ГК == 0).
"""
import numpy as np
import pandas as pd
...
class DefecturaDetector:
    ...
    def _get_threshold(self, kag) -> float:
        ...
        return eo / self.config.DEFECT_EO_DIVISOR
    ...
    def filter_eligible_kags(
        ...
        # Объединяем условия
        eligible = gk_ever_positive.index[
            gk_ever_positive.reindex(gk_ever_positive.index, fill_value=False)
        ].tolist()
        ...
        # Предзагружаем ЭО, чтобы в логах было сразу видно
        eo_map = self._load_eo_map()
        ...
        print(f"✅ КАГ с достаточной историей (≥{self.config.MIN_OBS_LAST30} наблюдений): {has_enough_history.sum():,}")
        print(f"🎯 ИТОГО подходящих КАГ: {len(eligible):,}")
        print(f"   из них с ЭО > 0: {eligible_with_eo:,}")
        print(f"   порог дефектуры: ГК < {self.config.DEFECT_EO_PCT:.0%} × ЭО (при ЭО ≥ {self.config.MIN_EO_FOR_PCT})")
        print(f"                    ГК == 0 (при ЭО < {self.config.MIN_EO_FOR_PCT})")
        print(f"{'='*60}\n")
    ...
    def find_last_point_defects(
        ...
        Дефектура = ГК < 10% ЭО  и  конкуренты > 0
        ...
        last_points['_threshold'] = last_points[self.config.COL_KAG].apply(_kag_threshold)
        ...
        # === ДИАГНОСТИКА — удалить потом ===
        print(f"\n🔍 ДИАГНОСТИКА find_last_point_defects:")
        print(f"   Строк в last_points: {len(last_points)}")
        print(f"   Тип COL_GK: {last_points[self.config.COL_GK].dtype}")
        print(f"   Тип _threshold: {last_points['_threshold'].dtype}")
        print(f"   Тип sum_competitors: {last_points['sum_competitors'].dtype}")
        print(f"   eo_map размер: {len(eo_map)}")
        print(f"   Пороги > 0: {(last_points['_threshold'] > 0).sum()}")
        print(f"   ГК == 0: {(last_points[self.config.COL_GK] == 0).sum()}")
        print(f"   0 < ГК < порог: {((last_points[self.config.COL_GK] > 0) & (last_points[self.config.COL_GK] < last_points['_threshold'])).sum()}")
        print(f"   sum_competitors > 0: {(last_points['sum_competitors'] > 0).sum()}")

        # Покажи 5 примеров где ГК > 0 но < порог
        _candidate = last_points[
            (last_points[self.config.COL_GK] > 0) & 
            (last_points['_threshold'] > 0) &
            (last_points[self.config.COL_GK] < last_points['_threshold'])
        ]
        print(f"   Кандидаты (0 < ГК < порог): {len(_candidate)}")
        if len(_candidate) > 0:
            print(_candidate[[self.config.COL_KAG, self.config.COL_GK, '_threshold', 'sum_competitors']].head(10).to_string())
        # === КОНЕЦ ДИАГНОСТИКИ ===
        ...
        print(f"🔴 КАГ в дефектуре (ГК < {self.config.DEFECT_EO_PCT:.0%}×ЭО, конкуренты>0): {len(result):,}")
    ...
    def find_defectura_episodes(
        ...
        print(f"   Порог: ГК < {self.config.DEFECT_EO_PCT:.0%} × ЭО (при ЭО ≥ {self.config.MIN_EO_FOR_PCT}, иначе ГК == 0)")
