...
class MetricsCalculator:
    """Расчёт метрик и обогащение данных"""
    ...
    def enrich_with_arrival_metrics(
        self, episodes: pd.DataFrame, kag_store: dict, work_date: pd.Timestamp,
    ) -> pd.DataFrame:
        """Обогащает эпизоды дефектуры метриками прихода"""
        rows = []

        for _, episode in tqdm(episodes.iterrows(), total=len(episodes), desc="Расчёт метрик прихода"):
            ...
            metrics = self.arrival_detector.detect_from_numpy_store(st, start_date, end_date, work_date)
        ...
        print(f"Метрики рассчитаны для {len(result):,} эпизодов, с приходом: {result['arrival_flag'].sum():,}")
        return result

    def add_detailed_arrival_stats(
        self, base_df: pd.DataFrame, full_data: pd.DataFrame, work_date: pd.Timestamp,
    ) -> pd.DataFrame:
        ...
        for _, rec in tqdm(base_df.iterrows(), total=len(base_df), desc="Детальная статистика прихода"):
            ...
            comp_events = self.arrival_detector.detect_for_all_competitors(one, deficit_start_date, end_date)

            row = {self.config.COL_KAG: kag}
            total_events = sum(len(events) for events in comp_events.values())
            total_volume = sum(sum(vol for _, vol in events) for events in comp_events.values())
            ...
            row['Конкуренты с приходами'] = (
                '; '.join(self.config.COMP_PRETTY.get(c, c) for c in comps_with_arrivals)
    ...
    def add_stock_snapshots(
        ...
        for kag in kags:
            ...
            for comp_col in self.config.COMP_COLS:
                ...
                row[f'Остаток {pretty} (вчера)'] = self._get_value_on_date(one, comp_col, yesterday)
            ...
            for comp_col in self.config.COMP_COLS:
                pretty = self.config.COMP_PRETTY.get(comp_col, comp_col)
                row[f'Остаток {pretty} (последняя дата)'] = self._get_value_on_date(one, comp_col, last_date)

            row['Остаток ГК (последняя дата)'] = self._get_value_on_date(one, self.config.COL_GK, last_date)
            rows.append(row)

        return pd.DataFrame(rows)

    def add_categories_and_metadata(
        self, df: pd.DataFrame, full_data: pd.DataFrame, work_date: pd.Timestamp,
    ) -> pd.DataFrame:
        """Добавляет категории, названия, статусы"""
        result = df.copy()

        if self.config.COL_KAG_NAME in full_data.columns:
            name_map = full_data.groupby(self.config.COL_KAG)[self.config.COL_KAG_NAME].first().to_dict()
        ...
        if 'defect_start_date' in result.columns:
            ...
            result['Категория'] = [CategoryConfig.categorize_by_days_ago(int(d)) for d in days_ago]
        else:
            ...
            for _, row in result.iterrows():
                ...
                deficit_dates.append(after_pos['date_n'].min() if not after_pos.empty else pd.NaT)
        ...
        if 'is_finished' in result.columns:
            result['Статус дефектуры'] = np.where(result['is_finished'], 'Закончившаяся', 'Активная')
        ...
        if 'defect_start_date' in result.columns and 'defect_end_date' in result.columns:
            result['Длительность дефектуры, дней'] = np.where(
                ...
                (pd.to_datetime(result['defect_end_date']) - pd.to_datetime(result['defect_start_date'])).dt.days,
        ...
        if 'arrival_first_date' in result.columns and 'defect_start_date' in result.columns:
            result['Лаг реакции, дней'] = (
                pd.to_datetime(result['arrival_first_date']) - pd.to_datetime(result['defect_start_date'])
            ).dt.days
    ...
    @staticmethod
    def _get_value_on_date(df: pd.DataFrame, column: str, target_date: pd.Timestamp) -> float:
        target_n = pd.to_datetime(target_date).normalize()
        mask = df['date_n'] == target_n
        if not mask.any():
            return 0.0
        return float(pd.to_numeric(df.loc[mask, column], errors='coerce').fillna(0).iloc[-1])
